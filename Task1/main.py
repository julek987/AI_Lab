...
def main():
    ...
    for n in test_sizes:
        ...
        for scenario_connection, scenario_symmetry in scenarios:
            ...
            # BFS TSP
            try:
                route_bfs, cost_bfs, time_bfs, mem_bfs = test_algorithm(bfs, graph)
            ...
            # DFS TSP
            try:
                route_dfs, cost_dfs, time_dfs, mem_dfs = test_algorithm(dfs, graph)
                print(f"  DFS => cost={cost_dfs}, time={time_dfs:.3f}s, mem={mem_dfs:.1f}KB")
            except:
                print("  DFS => too large, or an error occurred")

            # Nearest Neighbor
            route_nn, cost_nn, time_nn, mem_nn = test_algorithm(nearest_neighbor, graph)
            print(f"  NN  => cost={cost_nn}, time={time_nn:.3f}s, mem={mem_nn:.1f}KB")

            # Dijkstra-based
            route_djk, cost_djk, time_djk, mem_djk = test_algorithm(dijkstra_approx, graph)
            ...
            route_astar_adm, cost_astar_adm, time_astar_adm, mem_astar_adm = test_algorithm(a_star_admissible, graph)
            ...
            route_astar_inad, cost_astar_inad, time_astar_inad, mem_astar_inad = test_algorithm(a_star_inadmissible, graph)
